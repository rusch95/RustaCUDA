...
impl<T: DeviceCopy> DeviceBuffer<T> {
    ...
    /// let values = [0u64; 5];
    /// unsafe {
    ///     let mut buffer = DeviceBuffer::from_slice_async(&values, stream.inner).unwrap();
    /// }
    /// ```
    pub unsafe fn from_slice_async(slice: &[T], stream: CUstream) -> CudaResult<Self> {
        let mut uninit = DeviceBuffer::uninitialized(slice.len())?;
...
mod test_device_buffer {
    ...
    fn test_async_copy_to_from_device() {
        ...
        let mut end = [0u64, 0, 0, 0, 0, 0];
        unsafe {
            let buf = DeviceBuffer::from_slice_async(&start, stream.inner).unwrap();
            buf.async_copy_to(&mut end, stream.inner).unwrap();
        }
    ...
    fn test_async_slice() {
        ...
        let mut end = [0u64, 0];
        unsafe {
            let mut buf = DeviceBuffer::from_slice_async(&[0u64, 0, 0, 0], stream.inner).unwrap();
            buf.async_copy_from(&start[0..4], stream.inner).unwrap();
            buf[0..2].async_copy_to(&mut end, stream.inner).unwrap();
            stream.synchronize().unwrap();
            assert_eq!(start[0..2], end);
        }
    }
    ...
    fn test_async_copy_to_d2h_wrong_size() {
        ...
        let stream = Stream::new(StreamFlags::NON_BLOCKING, None).unwrap();
        unsafe {
            let buf = DeviceBuffer::from_slice_async(&[0u64, 1, 2, 3, 4, 5], stream.inner).unwrap();
            let mut end = [0u64, 1, 2, 3, 4];
            let _ = buf.async_copy_to(&mut end, stream.inner);
        }
    }
    ...
    fn test_async_copy_from_h2d_wrong_size() {
        ...
        unsafe {
            let mut buf =
                DeviceBuffer::from_slice_async(&[0u64, 1, 2, 3, 4, 5], stream.inner).unwrap();
            let _ = buf.async_copy_from(&start, stream.inner);
        }
    }
    ...
    fn test_async_copy_device_slice_to_device() {
        ...
        unsafe {
            let start =
                DeviceBuffer::from_slice_async(&[0u64, 1, 2, 3, 4, 5], stream.inner).unwrap();
            let mut mid = DeviceBuffer::from_slice_async(&[0u64, 0, 0, 0], stream.inner).unwrap();
            let mut end = DeviceBuffer::from_slice_async(&[0u64, 0], stream.inner).unwrap();
            let mut host_end = [0u64, 0];
            start[1..5].async_copy_to(&mut mid, stream.inner).unwrap();
            end.async_copy_from(&mid[1..3], stream.inner).unwrap();
            end.async_copy_to(&mut host_end, stream.inner).unwrap();
            stream.synchronize().unwrap();
            assert_eq!([2u64, 3], host_end);
        }
    }
    ...
    fn test_async_copy_to_d2d_wrong_size() {
        ...
        unsafe {
            let buf = DeviceBuffer::from_slice_async(&[0u64, 1, 2, 3, 4, 5], stream.inner).unwrap();
            let mut end =
                DeviceBuffer::from_slice_async(&[0u64, 1, 2, 3, 4], stream.inner).unwrap();
            let _ = buf.async_copy_to(&mut end, stream.inner);
        }
    }
    ...
    fn test_async_copy_from_d2d_wrong_size() {
        ...
        unsafe {
            let mut buf =
                DeviceBuffer::from_slice_async(&[0u64, 1, 2, 3, 4, 5], stream.inner).unwrap();
            let start = DeviceBuffer::from_slice_async(&[0u64, 1, 2, 3, 4], stream.inner).unwrap();
            let _ = buf.async_copy_from(&start, stream.inner);
        }
    }
